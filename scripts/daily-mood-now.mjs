...
async function main() {
  ...
  // Pre-commit any pending cache updates (github-pins, footer-gallery) to avoid blocking.
  // These are generated by `npm run build` and may drift between runs.
  const cacheFiles = ['src/data/github-pins.json', 'src/data/footer-gallery.json'];
  const pendingCache = cacheFiles.filter((f) => {
    const r = spawnSync('git', ['status', '--porcelain', f], { encoding: 'utf8' });
    return r.status === 0 && String(r.stdout || '').trim();
  });

  if (pendingCache.length > 0) {
    console.log(`[daily-mood] Auto-committing cache updates: ${pendingCache.join(', ')}`);
    sh('git', ['add', ...pendingCache]);
    sh('git', ['commit', '-m', 'chore: auto-update cache files']);
  }

  // Must be clean for the main commit.
